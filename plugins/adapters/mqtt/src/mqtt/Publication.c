...
RTI_MQTT_Publication_initialize(
...
DDS_ReturnCode_t
RTI_MQTT_Publication_new(struct RTI_MQTT_Client *client,
                              RTI_MQTT_PublicationConfig *config,
                              struct RTI_MQTT_Publication **pub_out)
{
    ...
    *pub_out = NULL;
    ...
    pub = (struct RTI_MQTT_Publication*)
            RTI_MQTT_Heap_allocate(sizeof(struct RTI_MQTT_Publication));
    if (pub == NULL)
    {
...
void
RTI_MQTT_Publication_delete(struct RTI_MQTT_Publication *self)
{
    ...
    DDS_DynamicData *message)
{
    ...
    if (use_message_info)
    {
        if (!DDS_DynamicData_member_exists(
                message, "info", DDS_DYNAMIC_DATA_MEMBER_ID_UNSPECIFIED))
        {
            RTI_MQTT_LOG_PUBLICATION_WRITE_MESSAGE_INFO_NOT_FOUND(
                    self, message)
            goto done;
        }
        if (DDS_RETCODE_OK !=
    ...
    payload_buffer = 
            DDS_OctetSeq_get_contiguous_buffer(&self->req_ctx.payload);
    payload_len = DDS_OctetSeq_get_length(&self->req_ctx.payload);

    if (DDS_RETCODE_OK != 
            RTI_MQTT_Publication_write_w_params(self,
                                                payload_buffer,
                                                payload_len,
                                                self->req_ctx.topic,
                                                &params))
...
DDS_ReturnCode_t
RTI_MQTT_Publication_on_write_result(
        struct RTI_MQTT_Publication *self,
...
DDS_ReturnCode_t
RTI_MQTT_Publication_on_write_delivery_result(
        struct RTI_MQTT_Publication *self,
        DDS_ReturnCode_t result)
{
    DDS_ReturnCode_t retval = DDS_RETCODE_ERROR;
    ...
    if (self->req_ctx.last_write_qos != RTI_MQTT_QosLevel_TWO)
    {
        RTI_MQTT_LOG_PUBLICATION_UNEXPECTED_DELIVERY_RESULT(
                self,self->req_ctx.last_write_qos)
        goto done;
    }
    ...
    RTI_MQTT_WriteParams *params)
{
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_Client_write_message(self->client, 
                                          self,
                                          buffer,
                                          buffer_len,
                                          topic,
                                          params))
    {
        RTI_MQTT_LOG_PUBLICATION_WRITE_MESSAGE_FAILED(
                self,topic,params->qos_level,params->retained,buffer)
        self->data->message_status->pending_count -= 1;
        goto done;
    }
    ...
    {
        RTI_MQTT_LOG_CLIENT_WAIT_FOR_WRITE_RESULTS_FAILED(
                    self->client, self)
        goto done;
    }
    ...
    else
    {
        ...
        if (topic_len <= self->req_ctx.topic_len)
        {
            RTI_MQTT_Memory_copy(
                self->req_ctx.topic, topic, sizeof(char) * topic_len);
...
RTIBool
RTI_MQTT_PublicationPtr_initialize_w_params(
    struct RTI_MQTT_Publication **self,
...
RTIBool
RTI_MQTT_PublicationPtr_finalize_w_params(
    struct RTI_MQTT_Publication **self,
