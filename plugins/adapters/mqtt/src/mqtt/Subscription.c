...
RTI_MQTT_Subscription_initialize(
    ...
    struct RTI_MQTT_Subscription **sub_out)
{
    ...
    *sub_out = NULL;
    ...
    sub = (struct RTI_MQTT_Subscription*)
            RTI_MQTT_Heap_allocate(sizeof(struct RTI_MQTT_Subscription));
    if (sub == NULL)
    {
...
void
RTI_MQTT_Subscription_delete(struct RTI_MQTT_Subscription *self)
{
    ...
    DDS_Boolean *lost_out)
{
    ...
    if (DDS_RETCODE_OK != 
            RTI_MQTT_MessageReceiveQueue_receive(self->queue,
                                                 buffer,
                                                 buffer_len,
                                                 topic,
                                                 msg_info,
    ...
    struct DDS_DynamicDataSeq *messages)
{
    ...
    if (DDS_RETCODE_OK != 
            RTI_MQTT_MessageReceiveQueue_read(
                    self->queue, max_messages, messages))
    {
        RTI_MQTT_LOG_SUBSCRIPTION_READ_FROM_QUEUE_FAILED(self, max_messages)
        goto done;
    }
    RTI_MQTT_TRACE_2("READ result:","%d messages [%p]",
        DDS_DynamicDataSeq_get_length(messages), self)
...
static DDS_ReturnCode_t
RTI_MQTT_Subscription_initialize(struct RTI_MQTT_Subscription *self,
                                 struct RTI_MQTT_Client *client,
                                 RTI_MQTT_SubscriptionConfig *config)
{
...
static RTIBool
RTI_MQTT_SubscriptionPtr_initialize_w_params(
    struct RTI_MQTT_Subscription **self,
...
static RTIBool
RTI_MQTT_SubscriptionPtr_finalize_w_params(
    struct RTI_MQTT_Subscription **self,
