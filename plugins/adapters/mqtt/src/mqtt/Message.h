...
DDS_SEQUENCE(RTI_MQTT_ReceivedMessagePtrSeq, 
             struct RTI_MQTT_ReceivedMessage*);

struct RTI_MQTT_MessageReceiveQueue;

typedef void
(*RTI_MQTT_MessageReceiveQueue_OnDataAvailableCallback)(
    struct RTI_MQTT_MessageReceiveQueue *queue, void *arg);
...
RTI_MQTT_TRACE_8((msg_),\
        "[cap=%u, size=%u, head=%u, next=%u, next_un=%u, queue.len=%u, queue.max=%u, loaned=%d]", \
        (q_)->capacity,\
        (q_)->size,\
        (q_)->head,\
        (q_)->next,\
