...
RTI_MQTT_Client_initialize(struct RTI_MQTT_Client *self,
                             struct RTI_MQTT_ClientConfig *config);

static DDS_ReturnCode_t
RTI_MQTT_Client_finalize(struct RTI_MQTT_Client *self);

static DDS_ReturnCode_t
RTI_MQTT_Client_set_state(struct RTI_MQTT_Client *self,
...
static DDS_ReturnCode_t
RTI_MQTT_Client_reconnect(struct RTI_MQTT_Client *self);
...
static DDS_ReturnCode_t
RTI_MQTT_Client_submit_subscriptions(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_PendingRequest *req);

static DDS_ReturnCode_t
RTI_MQTT_Client_cancel_subscriptions(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_PendingRequest *req);
...
static void
RTI_MQTT_Client_on_write_delivery_result(
    struct RTI_MQTT_PendingRequest *req,
    DDS_ReturnCode_t result);
...
static DDS_ReturnCode_t
RTI_MQTT_Client_delete_request(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_PendingRequest **req_out);

static void
RTI_MQTT_Client_handle_request_result(
    struct RTI_MQTT_PendingRequest *req,
    DDS_ReturnCode_t result);

static DDS_ReturnCode_t
RTI_MQTT_Client_create_publication_requests(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_Publication *pub);

static DDS_ReturnCode_t
RTI_MQTT_Client_create_subscription_requests(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_Subscription *sub);

static DDS_ReturnCode_t
RTI_MQTT_Client_delete_publication_requests(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_Publication *pub);

static DDS_ReturnCode_t
RTI_MQTT_Client_delete_subscription_requests(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_Subscription *sub);

static DDS_ReturnCode_t
RTI_MQTT_Client_add_subscription(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_Subscription *sub);

static DDS_ReturnCode_t
RTI_MQTT_Client_remove_subscription(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_Subscription *sub);

static DDS_ReturnCode_t
RTI_MQTT_Client_add_publication(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_Publication *pub);

static DDS_ReturnCode_t
RTI_MQTT_Client_remove_publication(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_Publication *pub);
...
static RTIBool
RTI_MQTT_PendingRequestPtr_initialize_w_params(
...
static RTIBool
RTI_MQTT_PendingRequestPtr_finalize_w_params(
    struct RTI_MQTT_PendingRequest **self,
...
static RTIBool
RTI_MQTT_PendingRequestPtr_copy(
    ...
    DDS_ReturnCode_t result)
{
    ...
    if (DDS_RETCODE_OK != 
            DDS_GuardCondition_set_trigger_value(
                    req->condition, DDS_BOOLEAN_TRUE))
    {
        RTI_MQTT_LOG_CLIENT_SET_GUARD_CONDITION_TRIGGER_FAILED(
                self, req->condition, DDS_BOOLEAN_TRUE)
        goto done;
    }
    ...
    {
        RTI_MQTT_ERROR_3("FAILED to handle request result:",
            "client=%p, req=%p, result=%d", self, req, result)

        if (DDS_RETCODE_OK != 
                RTI_MQTT_Client_set_state(
                    self, RTI_MQTT_ClientStateKind_ERROR, NULL))
        {
...
{
    DDS_ReturnCode_t retcode = DDS_RETCODE_ERROR;
    struct RTI_MQTT_PendingRequest *req = NULL,
                        **req_ref = NULL,
                        def_req = RTI_MQTT_PendingRequest_INITIALIZER;
    DDS_Boolean attached = DDS_BOOLEAN_FALSE,
                seq_cond_initd = DDS_BOOLEAN_FALSE,
    ...
    *req_out = NULL;
    ...
    req = (struct RTI_MQTT_PendingRequest*) 
            RTI_MQTT_Heap_allocate(sizeof(struct RTI_MQTT_PendingRequest));
    if (req == NULL)
    {
        /* TODO Log error */
        goto done;
    ...
    *req = def_req;
    ...
    if (DDS_RETCODE_OK != 
            DDS_WaitSet_attach_condition(req->waitset,
    ...
    *req_out = req;
    ...
    if (retcode != DDS_RETCODE_OK)
    {
        if (req != NULL)
        {
            ...
            {
                if (DDS_RETCODE_OK != 
                        DDS_WaitSet_detach_condition(req->waitset,
                            DDS_GuardCondition_as_condition(req->condition)))
                {
                    /* TODO Log error */
                }
...
static DDS_ReturnCode_t
RTI_MQTT_Client_wait_for_request(
    struct RTI_MQTT_Client *self,
    struct RTI_MQTT_PendingRequest *req)
{
    DDS_ReturnCode_t retcode = DDS_RETCODE_ERROR;
    ...
    retcode = DDS_WaitSet_wait(req->waitset,
                               &req->cond_seq,
                               &dds_timeout);
    ...
    if (DDS_RETCODE_OK != 
            DDS_GuardCondition_set_trigger_value(
                        req->condition,DDS_BOOLEAN_FALSE))
    {
        RTI_MQTT_LOG_CLIENT_SET_GUARD_CONDITION_TRIGGER_FAILED(
                self, req->condition, DDS_BOOLEAN_FALSE)
        goto done;
    }
    ...
    struct RTI_MQTT_Subscription *sub)
{
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_Client_delete_request(self, &sub->req_sub))
    {
        /* TODO Log error */
        goto done;
    }
...
RTI_MQTT_Client_on_connection_lost(struct RTI_MQTT_Client *self)
{
    ...
    if (DDS_RETCODE_OK != 
            RTI_MQTT_Client_set_state(
                self, RTI_MQTT_ClientStateKind_DISCONNECTED, NULL))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    RTI_MQTT_MessageInfo *msg_info)
{
    ...
    for (i = 0; i < seq_len; i++)
    {
        ...
        if (match)
        {
            RTI_MQTT_TRACE_3("DELIVER message:",
    ...
    struct RTI_MQTT_Publication *pub)
{
    ...
    if (DDS_RETCODE_OK != 
            RTI_MQTT_Client_wait_for_request(self, pub->req))
    {
    ...
    DDS_ReturnCode_t result)
{
    ...
    {
        RTI_MQTT_ERROR_3("connection ERROR","client=%p, req=%p result=%d",
                req->client, req, result)
    }
    else
    {
    ...
    DDS_ReturnCode_t result)
{
    ...
    if (result != DDS_RETCODE_OK)
    {
        RTI_MQTT_ERROR_3("disconnection ERROR","client=%p, req=%p, result=%d",
                req->client, req, result)
    }
    else
    {
        RTI_MQTT_LOG_2("disconnection OK","client=%p, req=%p",
                req->client, req)
    }
#endif /* RTI_MQTT_USE_LOG */
    ...
    DDS_ReturnCode_t result)
{
    ...
    if (result != DDS_RETCODE_OK)
    {
        RTI_MQTT_ERROR_3("subscription ERROR","client=%p, req=%p, result=%d",
                req->client, req, result)
    }
    else
    {
    ...
    DDS_ReturnCode_t result)
{
    ...
    if (result != DDS_RETCODE_OK)
    {
        RTI_MQTT_ERROR_3("unsubscription ERROR",
                "client=%p, req=%p, result=%d",
                req->client, req, result)
    }
    else
    {
        RTI_MQTT_LOG_2("unsubscription OK","client=%p, req=%p",
                req->client, req)
    }
#endif /* RTI_MQTT_USE_LOG */
    RTI_MQTT_Client_handle_request_result(req, result);
    ...
    DDS_ReturnCode_t result)
{
    ...
    if (result != DDS_RETCODE_OK)
    {
        RTI_MQTT_ERROR_4("write delivery ERROR", 
                "client=%p, pub=%p, req=%p, result=%d",
                self, pub, req, result)
    }
    else
    {
        RTI_MQTT_TRACE_3("write delivery OK",
    ...
    DDS_ReturnCode_t result)
{
    ...
    if (result != DDS_RETCODE_OK)
    {
        RTI_MQTT_ERROR_4("write ERROR", 
                "client=%p, pub=%p, req=%p, result=%d",
                self, pub, req, result)
    }
    else
    {
        RTI_MQTT_TRACE_3("write OK", "client=%p, pub=%p, req=%p",
                self, pub, req)
    }
#endif /* RTI_MQTT_USE_LOG */
...
DDS_ReturnCode_t
RTI_MQTT_Client_new(struct RTI_MQTT_ClientConfig *config,
...
RTI_MQTT_Client_connect(struct RTI_MQTT_Client *self)
{
    ...
    {
        RTI_MQTT_ERROR_2("invalid client state for connection:",
                "client=%p, state=%s",
                self, 
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_Client_set_state(
                    self, RTI_MQTT_ClientStateKind_CONNECTING, NULL))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_Client_set_state(
                    self, RTI_MQTT_ClientStateKind_CONNECTED, NULL))
    {
        /* TODO Log error */
        goto done;
    }
...
{
    DDS_ReturnCode_t retval = DDS_RETCODE_ERROR;
    DDS_Boolean unsubscribe = DDS_BOOLEAN_FALSE,
                locked = DDS_BOOLEAN_FALSE;
    ...
    {
        RTI_MQTT_TRACE_1("client ALREADY disconnected:","client=%s",
            self->data->config->id)
        retval = DDS_RETCODE_OK;
        goto done;
    }
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_Client_set_state(
                    self, RTI_MQTT_ClientStateKind_DISCONNECTING, NULL))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    RTI_MQTT_LOG_1("client DISCONNECTED","client=%s",
                    self->data->config->id)
        ...
        for (i = 0; i < topics_len; i++)
        {
            const char *topic = 
                *DDS_StringSeq_get_reference(
                        &sub->data->config->topic_filters,i);
                             ...
                             RTI_MQTT_ClientConfig *config)
{
    ...
    *self = def_self;
    ...
    {
        RTI_MQTT_LOG_COPY_DATA_FAILED("RTI_MQTT_ClientConfig",
...
RTI_MQTT_Client_finalize(struct RTI_MQTT_Client *self)
{
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_ClientMqttApi_Paho_delete_client(self))
    {
        /* TODO Log error */
        goto done;
    }
                            ...
                            RTI_MQTT_ClientStateKind *prev_state_out)
{
    ...
    RTI_MQTT_LOG_3("client STATE CHANGED","client=%s, state=%s, prev=%s",
            self->data->config->id, 
...
{
    DDS_ReturnCode_t retcode = DDS_RETCODE_ERROR;
    DDS_UnsignedLong t_filters_len = 0,
                     params_len = 0,
                     params_len_start = 0,
                     params_i = 0,
                     i = 0;
    ...
    struct RTI_MQTT_SubscriptionParamsSeq *params)
{
    ...
    subs_len = 
        RTI_MQTT_SubscriptionPtrSeq_get_length(&self->subscriptions);
        ...
        for (j = 0; j < params_len; j++)
        {
            if (!RTI_MQTT_SubscriptionParams_copy(
                    RTI_MQTT_SubscriptionParamsSeq_get_reference(
                            params, params_all_len + j),
                    RTI_MQTT_SubscriptionParamsSeq_get_reference(
                            &sub->req_ctx.params, j)))
            {
                /* TODO Log error */
                goto done;
            }
...
RTI_MQTT_Client_reconnect(struct RTI_MQTT_Client *self)
{
    ...
    {
        RTI_MQTT_LOG_1("SKIP reconnect:","client=%s",
            self->data->config->id)
        retval = DDS_RETCODE_OK;
        goto done;
    }
...
RTI_MQTT_Client_submit_all_subscriptions(struct RTI_MQTT_Client *self)
{
    ...
    if (!RTI_MQTT_SubscriptionParamsSeq_copy(
                &req_ctx_copy.params,&self->req_ctx_sub.params))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    struct RTI_MQTT_PendingRequest *req)
{
...
#if RTI_MQTT_USE_LOG
    {
        ...
        RTI_MQTT_LOG_2("SUBMIT subscriptions:","client=%p, subs=%d",
                self, seq_len)
        for (i = 0; i < seq_len; i++)
        {
            RTI_MQTT_SubscriptionParams *sub_p =
    ...
    struct RTI_MQTT_PendingRequest *req)
{
    ...
    {
        DDS_UnsignedLong i = 0;
        RTI_MQTT_LOG_2("CANCEL subscriptions:","client=%p, subs=%d",
                self, seq_len)
        for (i = 0; i < seq_len; i++)
        {
            RTI_MQTT_SubscriptionParams *sub_p =
    ...
    if (DDS_RETCODE_OK != 
            RTI_MQTT_Client_wait_for_request(self, req))
    {
    ...
    struct RTI_MQTT_Subscription *sub)
{
    ...
    if (!RTI_MQTT_SubscriptionPtrSeq_ensure_length(
            &self->subscriptions, seq_len + 1, seq_len + 1))
    {
    ...
    struct RTI_MQTT_Subscription *sub)
{
    ...
    for (i = rm_i; i + 1 < seq_len; i++)
    {
        *RTI_MQTT_SubscriptionPtrSeq_get_reference(
                        &self->subscriptions, i) = 
            *RTI_MQTT_SubscriptionPtrSeq_get_reference(
                        &self->subscriptions, i + 1);
    }
    if (!RTI_MQTT_SubscriptionPtrSeq_set_length(
            &self->subscriptions, seq_len - 1))
    {
    ...
    if (!RTI_MQTT_SubscriptionPtrSeq_set_maximum(
            &self->subscriptions, seq_len - 1))
    {
        RTI_MQTT_LOG_SET_SEQUENCE_MAX_FAILED(
                    &self->subscriptions, seq_len - 1)
        goto done;
    }
    ...
    struct RTI_MQTT_Publication *pub)
{
    ...
    if (!RTI_MQTT_PublicationPtrSeq_ensure_length(
            &self->publications, seq_len + 1, seq_len + 1))
    {
        RTI_MQTT_LOG_SET_SEQUENCE_ENSURE_LENGTH_FAILED(
                    &self->publications, seq_len + 1, seq_len + 1)
        goto done;
    ...
    struct RTI_MQTT_Publication *pub)
{
    ...
    for (i = rm_i; i + 1 < seq_len; i++)
    {
        *RTI_MQTT_PublicationPtrSeq_get_reference(
                        &self->publications, i) = 
            *RTI_MQTT_PublicationPtrSeq_get_reference(
                        &self->publications, i + 1);
    }
    if (!RTI_MQTT_PublicationPtrSeq_set_length(
            &self->publications, seq_len - 1))
    {
        RTI_MQTT_LOG_SET_SEQUENCE_LENGTH_FAILED(
                    &self->publications, seq_len - 1)
        goto done;
