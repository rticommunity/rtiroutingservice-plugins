...
DDS_ReturnCode_t
RTI_MQTT_ReceivedMessage_new(
        struct RTI_MQTT_ReceivedMessage **msg_out)
{
    DDS_ReturnCode_t retval = DDS_RETCODE_ERROR;
    struct RTI_MQTT_ReceivedMessage *msg = NULL;
    
    msg = (struct RTI_MQTT_ReceivedMessage*)
            RTI_MQTT_Heap_allocate(sizeof(struct RTI_MQTT_ReceivedMessage));
    if (msg == NULL)
    {
...
void
RTI_MQTT_ReceivedMessage_delete(
...
DDS_ReturnCode_t
RTI_MQTT_ReceivedMessage_initialize(
        struct RTI_MQTT_ReceivedMessage *self)
{
    self->message = NULL;
...
DDS_ReturnCode_t
RTI_MQTT_ReceivedMessage_finalize(
        struct RTI_MQTT_ReceivedMessage *self)
{
    DDS_ReturnCode_t retval = DDS_RETCODE_ERROR;
...
RTIBool
RTI_MQTT_ReceivedMessagePtr_initialize_w_params(
    struct RTI_MQTT_ReceivedMessage **self,
...
RTIBool
RTI_MQTT_ReceivedMessagePtr_finalize_w_params(
    struct RTI_MQTT_ReceivedMessage **self,
...
RTIBool
RTI_MQTT_ReceivedMessagePtr_copy(
...
{\
    RTI_MQTT_TRACE_1("queue.msg_status.RECEIVED =","%u",\
                    (q_)->msg_status->received_count) \
    RTI_MQTT_TRACE_1("queue.msg_status.UNREAD =","%u",\
                    (q_)->msg_status->unread_count) \
    RTI_MQTT_TRACE_1("queue.msg_status.READ =","%u",\
                    (q_)->msg_status->read_count) \
    RTI_MQTT_TRACE_1("queue.msg_status.LOST =","%u",\
    ...
    struct RTI_MQTT_MessageReceiveQueue **queue_out)
{
    ...
    queue = (struct RTI_MQTT_MessageReceiveQueue*)
            RTI_MQTT_Heap_allocate(
    ...
    if (DDS_RETCODE_OK != 
            RTI_MQTT_MessageReceiveQueue_initialize(
                                queue, size, msg_status))
    {
...
void
RTI_MQTT_MessageReceiveQueue_delete(
...
DDS_ReturnCode_t
RTI_MQTT_MessageReceiveQueue_initialize_circular(
    struct RTI_MQTT_MessageReceiveQueue *self)
{
    DDS_ReturnCode_t retval = DDS_RETCODE_ERROR;
    ...
    if (!RTI_MQTT_DDS_DynamicDataPtrSeq_set_maximum(
                &self->read_buffer, self->capacity))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    for (i = 0; i < self->capacity; i++)
    {
        ...
        msg_ref = RTI_MQTT_ReceivedMessagePtrSeq_get_reference(
                        &self->queue, i);
...
DDS_ReturnCode_t
RTI_MQTT_MessageReceiveQueue_initialize_unbounded(
    struct RTI_MQTT_MessageReceiveQueue *self)
{
    DDS_ReturnCode_t retval = DDS_RETCODE_ERROR;
    DDS_UnsignedLong i = 0,
...
RTI_MQTT_MessageReceiveQueue_initialize(
    ...
    RTI_MQTT_SubscriptionMessageStatus *msg_status)
{
    DDS_Boolean queue_initd = DDS_BOOLEAN_FALSE,
                lock_initd = DDS_BOOLEAN_FALSE;
    DDS_ReturnCode_t retval = DDS_RETCODE_ERROR;
    DDS_UnsignedLong i = 0,
                     initd_msgs = 0;
...
DDS_ReturnCode_t
RTI_MQTT_MessageReceiveQueue_finalize(
    struct RTI_MQTT_MessageReceiveQueue *self)
{
    DDS_ReturnCode_t retval = DDS_RETCODE_ERROR;
    ...
    DDS_Boolean *lost_out)
{
    ...
    if (!RTI_MQTT_ReceivedMessagePtrSeq_ensure_length(
            &self->queue, seq_len + 1, seq_len + 1))
    {
    ...
    struct DDS_DynamicDataSeq *messages)
{
    ...
    for (i = 0, idx = self->next_unread;
         i < max_messages && idx != self->next; 
         i++, idx = (self->next_unread + i) % self->capacity)
    {
        ...
        if (loan)
        {
            *RTI_MQTT_DDS_DynamicDataPtrSeq_get_reference(
                    &self->read_buffer,i) = rcvd_msg->message;
    ...
    struct DDS_DynamicDataSeq *messages)
{
    ...
    else
    {
        ...
        if (!RTI_MQTT_DDS_DynamicDataPtrSeq_ensure_length(
                    &self->read_buffer, seq_len, seq_len))
        {
            /* TODO Log error */
            goto done;
        }
        if (!RTI_MQTT_DDS_DynamicDataPtrSeq_set_length(
                        &self->read_buffer, seq_len))
        {
            /* TODO Log error */
            goto done;
        }
    ...
    if (seq_len < tot_messages)
    {
        RTI_MQTT_Memory_move(
            RTI_MQTT_ReceivedMessagePtrSeq_get_contiguous_buffer(&self->queue),
            RTI_MQTT_ReceivedMessagePtrSeq_get_reference(
                                            &self->queue, tot_messages),
            sizeof(struct RTI_MQTT_ReceivedMessage*)*(seq_len - tot_messages));
    }

    if (!RTI_MQTT_ReceivedMessagePtrSeq_set_length(
                    &self->queue, seq_len - tot_messages))
    {
        RTI_MQTT_LOG_SET_SEQUENCE_LENGTH_FAILED(
                    &self->queue, seq_len - tot_messages)
        goto done;
...
DDS_ReturnCode_t
RTI_MQTT_MessageReceiveQueue_return_loan(
    struct RTI_MQTT_MessageReceiveQueue *self,
    struct DDS_DynamicDataSeq *messages)
{
    DDS_ReturnCode_t retval = DDS_RETCODE_ERROR;
...
DDS_ReturnCode_t
RTI_MQTT_MessageReceiveQueue_set_listener(
    struct RTI_MQTT_MessageReceiveQueue *self,
