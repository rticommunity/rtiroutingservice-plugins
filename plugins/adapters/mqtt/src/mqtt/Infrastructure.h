...
DDS_ReturnCode_t
RTI_MQTT_Time_normalize(RTI_MQTT_Time *self);

DDS_ReturnCode_t
RTI_MQTT_Time_to_seconds(RTI_MQTT_Time *self, int *seconds_out);

DDS_ReturnCode_t
RTI_MQTT_Time_to_dds_time(
    RTI_MQTT_Time *self, struct DDS_Time_t *time_out);

DDS_ReturnCode_t
RTI_MQTT_Time_to_dds_duration(
    RTI_MQTT_Time *self, struct DDS_Duration_t *dur_out);

#define RTI_MQTT_Time_is_zero(t_) \
    ((t_)->seconds == 0 && (t_)->nanoseconds == 0)

#define RTI_MQTT_Time_is_equal(s_,o_) \
(((s_) == NULL && (o_) == NULL) || \
    ((s_) != NULL && (o_) != NULL && \
     (s_)->seconds == (o_)->seconds && \
     (s_)->nanoseconds == (o_)->nanoseconds))

DDS_ReturnCode_t
RTI_MQTT_DDS_OctetSeq_to_string(struct DDS_OctetSeq *self, char **str_out);


DDS_ReturnCode_t
RTI_MQTT_TopicFilter_match(const char *filter,
                             const char *value,
...
DDS_ReturnCode_t
RTI_MQTT_QosLevel_from_mqtt_qos(int mqtt_qos, RTI_MQTT_QosLevel *level_out);

DDS_Boolean
RTI_MQTT_QosLevel_is_valid(RTI_MQTT_QosLevel level);

#define RTI_MQTT_QosLevel_is_valid(q_) \
(\
    (q_) == RTI_MQTT_QosLevel_ZERO || \
...
(\
    ...
    ((q_) == RTI_MQTT_QosLevel_ONE)? "1" : \
    ((q_) == RTI_MQTT_QosLevel_TWO)? "2" : \
    "Unknown" \
)

#define RTI_MQTT_ClientStateKind_as_string(s_) \
(\
    ((s_) == RTI_MQTT_ClientStateKind_CONNECTED)? "CONNECTED" : \
    ((s_) == RTI_MQTT_ClientStateKind_DISCONNECTED)? "DISCONNECTED" : \
    ((s_) == RTI_MQTT_ClientStateKind_DISCONNECTING)? "DISCONNECTING" : \
    ((s_) == RTI_MQTT_ClientStateKind_CONNECTING)? "CONNECTING" : \
    ((s_) == RTI_MQTT_ClientStateKind_ERROR)? "ERROR" : "unknown" \
)

DDS_ReturnCode_t
RTI_MQTT_ClientStatus_new(DDS_Boolean allocate_optional,
                         RTI_MQTT_ClientStatus **state_out);

void
RTI_MQTT_ClientStatus_delete(RTI_MQTT_ClientStatus *self);

DDS_ReturnCode_t
RTI_MQTT_SubscriptionStatus_new(DDS_Boolean allocate_optional,
                               RTI_MQTT_SubscriptionStatus **state_out);

void
RTI_MQTT_SubscriptionStatus_delete(RTI_MQTT_SubscriptionStatus *self);

DDS_ReturnCode_t
RTI_MQTT_PublicationStatus_new(DDS_Boolean allocate_optional,
                              RTI_MQTT_PublicationStatus **state_out);

void
RTI_MQTT_PublicationStatus_delete(RTI_MQTT_PublicationStatus *self);

RTIBool
RTI_MQTT_MessagePtr_initialize_w_params(
    struct RTI_MQTT_Message **self,
    const struct DDS_TypeAllocationParams_t * allocParams);

RTIBool
RTI_MQTT_MessagePtr_finalize_w_params(
    struct RTI_MQTT_Message **self,
    const struct DDS_TypeDeallocationParams_t * deallocParams);

RTIBool
RTI_MQTT_MessagePtr_copy(
...
#define RTI_MQTT_PendingRequest_handle_result(r_,res_) \
    ...
    {\
        RTI_MQTT_ERROR_2("no result handler for request:",\
                "req=%p, result=%d",(r_), (res_))\
    }
