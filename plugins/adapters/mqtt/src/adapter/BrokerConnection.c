    ...
    struct RTI_RS_MQTT_BrokerConnection **connection_out)
{
    ...
    conn = (struct RTI_RS_MQTT_BrokerConnection*)
            RTI_MQTT_Heap_allocate(sizeof(struct RTI_RS_MQTT_BrokerConnection));
    if (conn == NULL)
    {
        /* TODO Log error */
        goto done;
    ...
    if (DDS_RETCODE_OK != 
            RTI_RS_MQTT_MessageReader_new_discovery(conn, 
...
void
RTI_RS_MQTT_BrokerConnection_delete(struct RTI_RS_MQTT_BrokerConnection *self)
{
...
RTI_RS_MQTT_BrokerConnection_validate_stream(
    ...
    const struct RTI_RoutingServiceStreamInfo *stream_info)
{
    ...
    if (stream_info->type_info.type_representation_kind !=
        ...
        (RTI_MQTT_String_compare(
            ...
            RTI_MQTT_MessageTypeSupport_get_type_name()) != 0 &&
        RTI_MQTT_String_compare(
            stream_info->type_info.type_name,
            RTI_MQTT_KeyedMessageTypeSupport_get_type_name()) != 0))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    RTI_RoutingServiceEnvironment * env)
{
    ...
    cur_reader_len = 
        RTI_RS_MQTT_MessageReaderPtrSeq_get_length(&self->readers);
    
    if (DDS_RETCODE_OK !=
            RTI_RS_MQTT_MessageReader_new(
                self, listener, properties, env, &reader))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    if (!RTI_RS_MQTT_MessageReaderPtrSeq_ensure_length(
            &self->readers, cur_reader_len+1, cur_reader_len+1))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    RTI_RoutingServiceEnvironment * env)
{
    ...
    {
        if (*RTI_RS_MQTT_MessageReaderPtrSeq_get_reference(
    ...
    RTI_RoutingServiceEnvironment * env)
{
    ...
    cur_writer_len = 
        RTI_RS_MQTT_MessageWriterPtrSeq_get_length(&self->writers);
    ...
    if (!RTI_RS_MQTT_MessageWriterPtrSeq_ensure_length(
            &self->writers, cur_writer_len+1, cur_writer_len+1))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    RTI_RoutingServiceEnvironment * env)
{
    ...
    for (i = 0; i < seq_len && !do_rm; i++)
    {
        if (*RTI_RS_MQTT_MessageWriterPtrSeq_get_reference(
...
void
RTI_RS_MQTT_BrokerConnection_update(
...
RTIBool
RTI_RS_MQTT_BrokerConnectionPtr_finalize_w_params(
    struct RTI_RS_MQTT_BrokerConnection **self,
...
RTIBool
RTI_RS_MQTT_BrokerConnectionPtr_copy(
...
#define T_initialize_w_params   RTI_RS_MQTT_BrokerConnectionPtr_initialize_w_params
