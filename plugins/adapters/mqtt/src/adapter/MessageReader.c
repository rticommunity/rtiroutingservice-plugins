...
RTI_RS_MQTT_MessageReader_new_internal(
    ...
    struct RTI_RS_MQTT_MessageReader **reader_out)
{
    ...
    reader = (struct RTI_RS_MQTT_MessageReader*)
            RTI_MQTT_Heap_allocate(sizeof(struct RTI_RS_MQTT_MessageReader));
    if (reader == NULL)
    {
        /* TODO Log error */
        goto done;
    ...
    {
        if (DDS_RETCODE_OK !=
                RTI_MQTT_Client_subscribe(reader->connection->client,
                                        &reader->config->sub,
                                        &reader->sub))
        {
            /* TODO Log error */
    ...
    struct RTI_RS_MQTT_MessageReader **reader_out)
{
    ...
    return RTI_RS_MQTT_MessageReader_new_internal(connection,
                                                  listener,
                                                  properties,
                                                  env,
                                                  DDS_BOOLEAN_TRUE,
...
void
RTI_RS_MQTT_MessageReader_delete(struct RTI_RS_MQTT_MessageReader *reader)
{
...
void
RTI_RS_MQTT_MessageReader_update(
    ...
    RTI_RoutingServiceEnvironment * env)
{
    ...
    DDS_UnsignedLong tot_streams = 0,
                     i = 0,
                     allocd_messages = 0;
    ...
    *count = 0;
    ...
    samples_list = (struct RTI_RoutingServiceStreamInfo**) 
    ...
    RTI_MQTT_Memory_zero(samples_list, 
                sizeof(struct RTI_RoutingServiceStreamInfo*)*tot_streams);
...
void
RTI_RS_MQTT_MessageReader_read(
    ...
    RTI_RoutingServiceEnvironment * env)
{
    ...
    DDS_Boolean messages_read = DDS_BOOLEAN_FALSE;
    DDS_UnsignedLong allocd_messages = 0,
                     messages_in_len = 0;
    ...
    if (RTI_RS_MQTT_MessageReader_is_discovery(self))
    {
        return RTI_RS_MQTT_MessageReader_read_discovery(
            self, samples_list_out, info_list_out, count, env);
    }
    ...
    *count = 0;
    ...
    if (DDS_RETCODE_OK != 
            RTI_MQTT_Subscription_read(self->sub,
    ...
    samples_list = 
        DDS_DynamicDataSeq_get_discontiguous_buffer(&messages);
    if (samples_list == NULL)
    {
        /* TODO Log error */
        goto done;
    ...
    if (!DDS_SampleInfoSeq_ensure_length(
            &self->info_seq, allocd_messages, allocd_messages))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    if (!RTI_MQTT_DDS_SampleInfoPtrSeq_ensure_length(
            &self->info_ptr_seq, allocd_messages, allocd_messages))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    RTI_RoutingServiceEnvironment * env)
{
    ...
    if (!DDS_DynamicDataSeq_loan_discontiguous(
                &messages, samples_list, count, count))
    {
        /* TODO Log error */
        goto done;
    }
...
RTIBool
RTI_RS_MQTT_MessageReaderPtr_initialize_w_params(
    struct RTI_RS_MQTT_MessageReader **self,
...
RTIBool
RTI_RS_MQTT_MessageReaderPtr_finalize_w_params(
    struct RTI_RS_MQTT_MessageReader **self,
...
RTIBool
RTI_RS_MQTT_MessageReaderPtr_copy(
...
RTIBool
RTI_MQTT_DDS_SampleInfoPtr_initialize_w_params(
...
RTIBool
RTI_MQTT_DDS_SampleInfoPtr_finalize_w_params(
    struct DDS_SampleInfo **self,
    const struct DDS_TypeDeallocationParams_t * deallocParams)
{
    *self = NULL;
...
RTIBool
RTI_MQTT_DDS_SampleInfoPtr_copy(
