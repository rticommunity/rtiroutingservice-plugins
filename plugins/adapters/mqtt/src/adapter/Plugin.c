        ...
        struct RTI_RS_MQTT_AdapterPlugin **plugin_out)
{
    ...
    plugin = (struct RTI_RS_MQTT_AdapterPlugin*)
                RTI_MQTT_Heap_allocate(sizeof(struct RTI_RS_MQTT_AdapterPlugin));
    if (plugin == NULL)
    {
        /* TODO Log error */
        goto done;
    ...
    plugin->parent.adapter_plugin_delete = 
    ...
    plugin->parent.connection_update = 
...
#endif
    plugin->parent.stream_reader_read = 
                    RTI_RS_MQTT_MessageReader_read;
    plugin->parent.stream_reader_return_loan = 
                    RTI_RS_MQTT_MessageReader_return_loan;
    plugin->parent.stream_reader_update = 
                    RTI_RS_MQTT_MessageReader_update;
    plugin->parent.stream_writer_write = 
                    RTI_RS_MQTT_MessageWriter_write;
    plugin->parent.stream_writer_update = 
                    RTI_RS_MQTT_MessageWriter_update;
    ...
    RTI_RoutingServiceEnvironment *env)
{
    ...
    RTI_MQTT_LOG_2("create CONNECTION","rs=%s, rs_group=%s",
    ...
    if (DDS_RETCODE_OK !=
            RTI_RS_MQTT_BrokerConnection_new(input_stream_discovery_listener,
    ...
    if (!RTI_RS_MQTT_BrokerConnectionPtrSeq_ensure_length(
            &self->connections, cur_conn_len+1, cur_conn_len+1))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    conn_ref = RTI_RS_MQTT_BrokerConnectionPtrSeq_get_reference(
                                    &self->connections, cur_conn_len);
    
    *conn_ref = conn;

    RTI_MQTT_LOG_2("new CONNECTION created:","adapter=%p, connection=%p", 
            self, conn)
