 ...
 * 
 * The MQTT Adapter exposes some macros which allow users to more easily write
 ...
 * 
 * Users should define this flag is they want to establish SSL/TLS connections
 * with MQTT Brokers.
 * 
 * Flag @ref RTI_MQTT_USE_SSL will be automatically enabled if this flag is 
 * set.
 * 
 ...
 * 
 * @brief Flag that can be used to guard code related to SSL/TLS functionality.
 * 
 * This flag is automatically defined based on the presence (or absence) of
 ...
 * @ingroup RtiMqtt_Compiler_User
 * 
 * @brief Compiler flag that enable internal use of static DDS types instead
 * of `DDS_DynamicData`.
 * 
 * Users should define this flag is they prefer the MQTT Adapter to use 
 * static DDS types `RTI_MQTT_Message`, and `RTI_MQTT_KeyedMessage`, instead
 * of `DDS_DynamicData`.
 * 
 * If static types are used internally, an additional copy will be performed
 * when data is passed to RTI Routing Service (which requires samples to be
 * represented as `DDS_DynamicData`).
 * 
 * Flag @ref RTI_MQTT_USE_STATIC_TYPES will be automatically enabled if this 
 * flag is set.
 * 
 ...
 */
#define RTI_MQTT_PROPERTY_PREFIX_CLIENT \
        "client."
 ...
 */
#define RTI_MQTT_PROPERTY_PREFIX_SUBSCRIPTION \
        "subscription."
 ...
 */
#define RTI_MQTT_PROPERTY_PREFIX_PUBLICATION \
        "publication."
 ...
 */
#define RTI_MQTT_PROPERTY_CLIENT_ID \
        RTI_MQTT_PROPERTY_PREFIX_CLIENT "id"
...
/**
 * @brief Configuration property to specify a list of MQTT Broker URIs to which
 * an `RTI_MQTT_Client` will try to connect.
 * 
 ...
 * maximum timeout for establishing a connection between an `RTI_MQTT_Client`,
 * and an MQTT Broker.
 */
...
/**
 * @brief Configuration property to select whether an `RTI_MQTT_Client` should
 * set the "clean session" flag when establishing a connection to an MQTT 
 * Broker.
*/
 ...
 * should delete all the subscriptions it created before disconnecting from an
 * MQTT Broker.
 */
...
/**
 * @brief Common prefix for configuration properties which control the maximum
 * time for which an `RTI_MQTT_Client` will wait to receive a reply from an
 * MQTT Broker.
 */
...
/**
 * @brief Configuration property controlling whether an `RTI_MQTT_Client` will
 * try to re-establish a connection to an MQTT Broker when an existing one
 * is lost.
 */
...
/**
 * @brief Configuration property specifying the path where an `RTI_MQTT_Client`
 * will store persistent MQTT session data.
 */
...
/**
 * @brief Configuration property to specify the path of a PEM file containing
 * the certificate of the Certificate Authoritative that generated the 
 * security material used by an `RTI_MQTT_Client` to establish an SSL/TLS
...
/**
 * @brief Configuration property to specify the path of a PEM file containing
 * the public certificate used by an `RTI_MQTT_Client` to establish an SSL/TLS
 * connection to an MQTT Broker.
 */
...
/**
 * @brief Configuration property to specify the path of a PEM file containing
 * the private key used by an `RTI_MQTT_Client` to establish an SSL/TLS
 * connection to an MQTT Broker.
 */
...
/**
 * @brief Configuration property to specify the password used to decrypt the
 * private key.
 */
...
/**
 * @brief Configuration property to specify the set of cyper suites allowed
 * by the SSL/TLS connection. 
 * 
 * See <a href="https://www.openssl.org/docs/manmaster/man1/ciphers.html#CIPHER_LIST_FORMAT">
 * OpenSSL's documentation</a> for more information on the format of this 
 * string.
 * 
 * The default value is "ALL", that is, all available cipher suites, excluding
 * those offering no encryption.
 */
...
/**
 * @brief Configuration property to specify the SSL/TLS protocol version to
 * use.
 */
...
/**
 * @brief Configuration property to control whether the certificate provided
 * by the Broker should be verified using the Certificate Authority's
 * certificate.
 */
...
/**
 * @brief Configuration property to specify the MQTT Qos level used to publish
 * all messages written by an `RTI_MQTT_Publication`.
 */
...
/**
 * @brief Configuration property to control whether an `RTI_MQTT_Publication`
 * should determine "topic", "qos", and "retained" information for each 
 * written message from the message's "info" attribute, or from the
 * `RTI_MQTT_Publication`'s other properties.
 */
...
/**
 * @brief Configuration property to specify the nanoseconds components of the 
 * maximum time for which an `RTI_MQTT_Publication` will wait to receive an 
 * acknowledgement to a message published with MQTT Qos 1 or 2.
 */
 ...
 * 
 * The MQTT Adapter includes a static logging module that can be enabled or
 * disabled at compile-time.
 * 
 * When enabled, the logging module can optionally produce more verbose output
 * by enabling trace-level logging.
 * 
 * By default, the logging module will be automatically enabled if `NDEBUG`
 * and @ref RTI_MQTT_DISABLE_LOG are not defined.
 * 
 ...
 * the logging module if `NDEBUG` is not defined (i.e. for debug builds).
 * 
...
#ifndef NDEBUG
#ifndef RTI_MQTT_DISABLE_LOG
#define RTI_MQTT_ENABLE_LOG
#endif /* RTI_MQTT_DISABLE_LOG */
#endif /* NDEBUG */
