...
MqttAgent_thread_subscriber(void *arg)
{
    ...
    s = DDS_Entity_get_statuscondition(
    ...
    if (DDS_RETCODE_OK !=
            DDS_StatusCondition_set_enabled_statuses(s,
    ...
    while (!cond_exit)
    {
        ...
        if (cond_reader)
        {
            retcode = RTI_MQTT_KeyedMessageDataReader_take(self->reader,
                                                    &message_seq,
                                                    &info_seq,
                ...
                for (i = 0; i < msg_seq_len; i++)
                {
                    ...
                    if (info->valid_data)
                    {
                        ...
                        MqttAgent_print_message(
                            self, (RTI_MQTT_Message*)msg, DDS_BOOLEAN_TRUE);
                    }
                }
                if (DDS_RETCODE_OK !=
                        RTI_MQTT_KeyedMessageDataReader_return_loan(
                                self->reader, &message_seq, &info_seq))
                {
                    /* TODO Log error */
                    goto done;
                }
