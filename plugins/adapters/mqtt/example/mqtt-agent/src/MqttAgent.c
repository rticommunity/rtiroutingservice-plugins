...
void
MqttAgent_finalize(struct MqttAgent *self)
{
...
int
MqttAgent_initialize(struct MqttAgent *self
#if USE_APP_GEN
                     ...
                     )
{
    ...
    *str_ref = DDS_String_dup(MQTT_AGENT_PROFILES);
    ...
    self->dp = DDS_DomainParticipantFactory_create_participant_from_config(
                    DDS_TheParticipantFactory, participant_name);
    ...
    if (self->dp == NULL)
    {
        RTI_MQTT_ERROR_3("failed to create participant:",
    ...
    {
        RTI_MQTT_ERROR_2("failed to create input topic:",
                            "topic='%s', type='%s'",
                            MQTT_AGENT_TOPIC_INPUT, type_name)
        goto done;
    }
    ...
    {
        RTI_MQTT_ERROR_2("failed to create output topic:",
                            "topic='%s', type='%s'",
                            MQTT_AGENT_TOPIC_OUTPUT, type_name)
        goto done;
    }
...
int
MqttAgent_publish_message(struct MqttAgent *self)
{
    int rc = 1;
    ...
    if (!DDS_OctetSeq_ensure_length(&self->pub_msg->payload.data,
                               msg_len, msg_fmt_len))
    {
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_MessageDataWriter_write(
                self->writer, self->pub_msg, &DDS_HANDLE_NIL))
    {
...
void
MqttAgent_print_message(struct MqttAgent *self, 
                        ...
                        DDS_Boolean in)
{
    ...
    else
    {
        ...
        if (msg_payload == NULL)
        {
            RTI_MQTT_ERROR_1("failed to allocate message buffer:","len=%d",
                msg_payload_len + 1)
            goto done;
        }
        memcpy(msg_payload,
...
int
MqttAgent_main(struct MqttAgent *self)
{
    int rc = 1;
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_Thread_spawn(
                MqttAgent_thread_publisher, self, &self->thread_pub))
    {
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_Thread_spawn(
                MqttAgent_thread_subscriber, self, &self->thread_sub))
    {
