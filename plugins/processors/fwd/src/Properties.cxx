    ...
    std::string& member_value)
{
    ...
    if (&member_json_value != &json_value_none)
    {
        ...
        std::string member_value_str(
            member_json_value.u.string.ptr, member_json_value.u.string.length);
...
{
    RTI_PRCS_FWD_LOG_FN(rti::prcs::fwd::property::parse_from_json<MatchingTable>)
    ...
    RTI_PRCS_FWD_LOG_1("JSON Matching Table:","%d entries",
    ...
    for (int i = 0; i < table_json_value->u.array.length; i++)
    {
        ...
        RTI_PRCS_FWD_LOG_2(" - JSON matching entry:","in_key=%s, out_name=%s",
            entry.in_key().c_str(), entry.out_name().c_str())
...
void
property::parse_config(
    const PropertySet& properties,
    ByInputNameForwardingEngineConfiguration& config)
{
    RTI_PRCS_FWD_LOG_FN(rti::prcs::fwd::property::parse_config<ByInputNameForwardingEngineConfiguration>)
    parse_matching_table(
            config.fwd_table(),
            properties,
...
{
    RTI_PRCS_FWD_LOG_FN(rti::prcs::fwd::property::parse_config<ByInputValueForwardingEngineConfiguration>)
