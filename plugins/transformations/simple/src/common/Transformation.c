...
const RTI_TSFM_TransformationConfig 
    ...
    RTI_TSFM_TransformationKind *kind_out)
{
    ...
    if (RTI_TSFM_String_compare(str,"serialize") == 0 ||
        ...
        RTI_TSFM_String_compare(str,"S") == 0 ||
        RTI_TSFM_String_compare(str,"RTI_TSFM_TransformationKind_SERIALIZER") == 0)
    {
        *kind_out = RTI_TSFM_TransformationKind_SERIALIZER;
        return DDS_RETCODE_OK;
    }
    else if (RTI_TSFM_String_compare(str,"deserialize") == 0 ||
        RTI_TSFM_String_compare(str,"deserializer") == 0 ||
        RTI_TSFM_String_compare(str,"d") == 0 ||
        RTI_TSFM_String_compare(str,"D") == 0 ||
        RTI_TSFM_String_compare(str,"RTI_TSFM_TransformationKind_DESERIALIZER") == 0)
    {
    ...
    RTI_TSFM_TransformationConfig **config_out)
{
    ...
    if (DDS_RETCODE_OK != 
            RTI_TSFM_TransformationConfig_default(&config))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    RTI_TSFM_TransformationConfig **config_out)
{
    ...
    if (!RTI_TSFM_TransformationConfig_copy(
                config, &RTI_TSFM_TransformationConfig_DEFAULT))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    RTI_TSFM_TransformationConfig **config_out)
{
    ...
    config = RTI_TSFM_TransformationConfigTypeSupport_create_data_w_params(
                                                                &alloc_params);
    if (config == NULL)
    {
        /* TODO Log error */
    ...
    RTI_TSFM_TransformationConfig *self)
{
    ...
    RTI_TSFM_TransformationConfigTypeSupport_delete_data(self);
}
    ...
    RTI_TSFM_Transformation *self)
{
    ...
    for (i = 0; i < seq_len; i++)
    {
        DDS_DynamicData **data_ref =
            RTI_TSFM_DDS_DynamicDataPtrSeq_get_reference(&self->read_buffer,i);
    ...
    RTI_RoutingServiceEnvironment *env)
{
    ...
    {
        RTI_TSFM_ERROR_1("read ALREADY in progress on transformation:",
            "%p",self)
        goto done;
    }

    read_buffer_max = RTI_TSFM_DDS_DynamicDataPtrSeq_get_maximum(&self->read_buffer);

    out_samples_len =
        (read_buffer_max > in_count)? read_buffer_max : in_count;

    if (!RTI_TSFM_DDS_DynamicDataPtrSeq_ensure_length(
            &self->read_buffer, out_samples_len, out_samples_len))
    {
        /* TODO Log error */
        goto done;
    }
    ...
    out_samples = 
        RTI_TSFM_DDS_DynamicDataPtrSeq_get_contiguous_buffer(&self->read_buffer);
    if (out_samples == NULL)
    {
        /* TODO Log error */
        goto done;
    ...
    for (i = 0; i < in_count; i++)
    {
        ...
        {
            out_sample = 
                DDS_DynamicDataTypeSupport_create_data(self->tsupport);
    ...
    RTI_RoutingServiceEnvironment *env)
{
    ...
    if (!self->read_buffer_loaned)
    {
        RTI_TSFM_ERROR_1("read NOT in progress on transformation:",
            "%p",self)
    }
