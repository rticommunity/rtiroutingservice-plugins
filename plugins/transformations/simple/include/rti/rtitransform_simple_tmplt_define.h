...
#define TPluginState_delete             concat(TPluginState, TypeSupport_delete_data)
...
#define TPluginConfig_delete        concat(TPluginConfig, TypeSupport_delete_data)
#endif /* TPluginConfig_delete */
#ifndef TPluginConfig_parse
#define TPluginConfig_parse         concat(TPluginConfig, _parse_from_properties)
...
const struct RTI_RoutingServiceVersion TPlugin_VERSION =
{
    T_VERSION_MAJOR,
    T_VERSION_MINOR,
    T_VERSION_RELEASE,
...
TPlugin_create(const struct RTI_RoutingServiceProperties *properties, 
...
DDS_ReturnCode_t
TPlugin_new(const struct RTI_RoutingServiceProperties *properties, 
            RTI_RoutingServiceEnvironment *env,
            TPlugin **plugin_out)
{
    ...
    *plugin_out = NULL;
...
#else
#ifdef T_static
    user_plugin = RTI_TSFM_UserTypePlugin_create_static(&self->parent,
                                                        T_serialize,
                                                        T_deserialize);
    if (user_plugin == NULL)
    {
    ...
    self->parent.parent.transformation_plugin_delete = 
            TPlugin_delete;
    ...
    self->parent.parent.transformation_transform =
            T_transform;
    self->parent.parent.transformation_return_loan = 
            T_return_loan;
    self->parent.parent.transformation_update = 
            T_update;
    ...
    if (retcode != DDS_RETCODE_OK)
    {
        if (self != NULL)
        {
            ...
            else
            {
                ...
                if (user_plugin != NULL)
                {
                    RTI_TSFM_UserTypePlugin_delete_default(user_plugin, 
                                                           &self->parent);
                }
#endif /* T_static */
...
void
TPlugin_delete(struct RTI_RoutingServiceTransformationPlugin *plugin,
               RTI_RoutingServiceEnvironment *env)
{
    DDS_Boolean retval = DDS_BOOLEAN_FALSE;
    ...
    seq_len = RTI_TSFM_TransformationPtrSeq_get_length(
                                                &self->parent.transforms);
    if (seq_len > 0)
    {
        /* TODO Log error */
        RTI_TSFM_ERROR_1("DELETING with existing transformation on plugin:",
            "%d", seq_len)
    }
    ...
    RTI_RoutingServiceEnvironment *env)
{
    ...
    if (DDS_RETCODE_OK !=
            RTI_TSFM_TransformationPlugin_finalize_transformation(
                                &self->parent, &transform->parent, env))
    {
        goto done;
    }
...
#ifdef T_initialize
    if (DDS_RETCODE_OK != T_initialize(self,
                                       plugin,
                                       input_type_info,
                                       output_type_info,
                                       properties,
...
void
T_transform(RTI_RoutingServiceTransformation transformation,
            ...
            RTI_RoutingServiceEnvironment *env)
{
    ...
    if (DDS_RETCODE_OK !=
            RTI_TSFM_Transformation_transform(&self->parent,
                out_sample_lst,
                out_info_lst,
                out_count,
                in_sample_lst,
...
void
T_return_loan(RTI_RoutingServiceTransformation transformation,
              ...
              RTI_RoutingServiceEnvironment *env)
{
    ...
    if (DDS_RETCODE_OK !=
            RTI_TSFM_Transformation_return_loan(
                &self->parent, sample_lst, info_lst, count, env))
    {
        /* TODO Log error */
    }
#if RTI_TSFM_USE_MUTEX
...
void
T_update(RTI_RoutingServiceTransformation transformation,
         const struct RTI_RoutingServiceProperties *properties,
         RTI_RoutingServiceEnvironment *env)
{
    T *self = (T*)transformation;
    ...
    if (DDS_RETCODE_OK != RTI_TSFM_Transformation_update(
                                        &self->parent, properties, env))
    {
        /* TODO Log error */
    }
