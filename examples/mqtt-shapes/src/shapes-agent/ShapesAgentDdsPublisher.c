    ...
    while (!cond_exit)
    {
        ...
        if (cond_writer_msg)
        {
            ...
            if (writer_statuses & DDS_PUBLICATION_MATCHED_STATUS)
            {
                ...
                RTI_MQTT_LOG_5("mqtt_message_writer MATCHED:",
        ...
        if (matches_msg > 0)
        {
            RTI_MQTT_TRACE_1("PUBLISHING on mqtt_message_writer to:",
                "%d readers", matches_msg)
            ...
            ShapesAgent_print_mqtt_message(
                    self, self->pub_msg, DDS_BOOLEAN_FALSE);
        }
        else
        {
