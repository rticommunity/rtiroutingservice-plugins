...
static void
ShapesAgent_signal_handler(int sig);
    ...
    if (ShapesAgent_fv_exit_condition != NULL)
    {
        if (DDS_RETCODE_OK !=
                DDS_GuardCondition_set_trigger_value(
                        ShapesAgent_fv_exit_condition, DDS_BOOLEAN_TRUE))
        {
...
ShapesAgent_finalize(struct ShapesAgent *self)
{
    ...
    {
        DDS_GuardCondition_set_trigger_value(self->cond_exit,
                                             DDS_BOOLEAN_TRUE);
    }
    ...
    if (self->cond_exit != NULL)
    {
        DDS_WaitSet_detach_condition(self->waitset,
    ...
    if (self->dp != NULL)
    {
        ...
        DDS_DomainParticipantFactory_delete_participant(
                DDS_TheParticipantFactory, self->dp);
        self->dp = NULL;
...
ShapesAgent_initialize(struct ShapesAgent *self)
{
    ...
    if (DDS_RETCODE_OK !=
            DDS_DomainParticipantFactory_get_qos(
                DDS_TheParticipantFactory, &dpf_qos))
    {
    ...
    if (!DDS_StringSeq_ensure_length(
            &dpf_qos.profile.url_profile,1,1))
    {
    ...
    *str_ref = DDS_String_dup(SHAPES_AGENT_PROFILES);
    if (*str_ref == NULL)
    {
        RTI_MQTT_ERROR_1("failed to duplicate string:","%s",
    ...
    self->dp = DDS_DomainParticipantFactory_create_participant_from_config(
                    DDS_TheParticipantFactory, SHAPES_AGENT_PARTICIPANT);
    
    if (self->dp == NULL) {
        RTI_MQTT_ERROR_1("failed to lookup participant","%s",
    ...
    RTI_MQTT_LOG_1("looked up DomainParticipant:","%s",SHAPES_AGENT_PARTICIPANT)

    {
        DDS_DataReader *reader = NULL;
        reader = DDS_DomainParticipant_lookup_datareader_by_name(
                            self->dp, SHAPES_AGENT_READER_MQTT);
        if (reader == NULL)
        {
            RTI_MQTT_ERROR_1("failed to lookup reader","%s",
        ...
        {
            RTI_MQTT_ERROR_1("failed to narrow reader","%s",
                SHAPES_AGENT_READER_MQTT)
            goto done;
        }
        RTI_MQTT_LOG_1("looked up DataReader:","%s",
            SHAPES_AGENT_PARTICIPANT "::" SHAPES_AGENT_READER_MQTT)
    }
    {
        DDS_DataWriter *writer = NULL;
        writer = DDS_DomainParticipant_lookup_datawriter_by_name(
                            self->dp, SHAPES_AGENT_WRITER_MQTT);
        if (writer == NULL)
        {
            RTI_MQTT_ERROR_1("failed to lookup writer","%s",
        ...
        {
            RTI_MQTT_ERROR_1("failed to narrow writer","%s",
                SHAPES_AGENT_WRITER_MQTT)
            goto done;
        }
        RTI_MQTT_LOG_1("looked up DataWriter:","%s",
    ...
    if (self->topic_circles == NULL)
    {
        RTI_MQTT_ERROR_1("failed to clone string:","%s",
...

    {
        ...
        self->pub_msg = 
            RTI_MQTT_KeyedMessageTypeSupport_create_data_w_params(&alloc_params);
        if (self->pub_msg == NULL)
        {
    ...
    self->shape = DDS_DynamicData_new(ShapeType_get_typecode(),
...
static DDS_Long 
random_no(DDS_Long min, DDS_Long max)
{
...
ShapesAgent_publish_message(struct ShapesAgent *self)
{
    ...
    char *msg_buf = NULL,
         *msg_topic = NULL,
         *cur_topic = NULL;
    DDS_UnsignedLong msg_fmt_len = 0,
                     msg_len = 0;
    DDS_Long x = 0,
             y = 0,
             size = 0;
    ...
    msg_buf = 
        DDS_OctetSeq_get_contiguous_buffer(&self->pub_msg->payload.data);
    if (msg_buf == NULL)
    {
        RTI_MQTT_ERROR_1("failed to get message buffer",
            "(%d)", self->msg_sent_tot)
    }
    ...
    if (!DDS_OctetSeq_set_length(
            ...
            msg_len))
    {
        RTI_MQTT_ERROR_1("failed to set payload length for message",
            "(%d)", self->msg_sent_tot)
        goto done;
    }
                               ...
                               DDS_Boolean in)
{
    ...
    else
    {
        ...
        if (DDS_RETCODE_OK != 
                RTI_TSFM_Json_FlatTypeTransformation_deserialize_shape(
                    msg_payload, msg_payload_len, self->shape))
        {
    ...
    else
    {
        printf("%s[%d] '%s' [%s%s%s]\n", 
            prefix, msg_tot, msg->topic,
            (msg->info->qos_level == RTI_MQTT_QosLevel_ZERO)? "QOS_0" :
            (msg->info->qos_level == RTI_MQTT_QosLevel_ONE)? "QOS_1" :
            (msg->info->qos_level == RTI_MQTT_QosLevel_TWO)? "QOS_2" : "QOS_UNKNOWN",
...
void
ShapesAgent_print_shape(struct ShapesAgent *self, 
                        void *entity,
                        ShapeType *shape,
                        DDS_Boolean in)
{
...
ShapesAgent_main(struct ShapesAgent *self)
{
    ...
    if (DDS_RETCODE_OK != 
            DDS_WaitSet_attach_condition(self->waitset,
                DDS_GuardCondition_as_condition(self->cond_exit)))
    {
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_Thread_spawn(
                ShapesAgent_thread_publisher, self, &self->thread_pub))
    {
    ...
    if (DDS_RETCODE_OK !=
            RTI_MQTT_Thread_spawn(
                ShapesAgent_thread_subscriber, self, &self->thread_sub))
    {
    ...
    while (!agent_done)
    {
        ...
        retcode = DDS_WaitSet_wait(
                    self->waitset,&self->active_conditions,&wait_timeout);
        ...
        {
            DDS_Condition *cond =
                        *DDS_ConditionSeq_get_reference(
                                        &self->active_conditions, i);
